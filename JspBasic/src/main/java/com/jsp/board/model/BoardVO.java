...
/*
	모델(model) 계층은 비즈니스 로직을 처리하는 역할을 담당 합니다.
	비즈니스 로직이란, 외부 프로그램과 연계 작업 등을 수행하는 로직입니다. (DB 등 ...)
	
	VO (value object) : 
	데이터베이스에서 조회된 값, 서버 프로그램에서 요청과 함께 얻은 값 등을 포장해서 운반하는 값에 관련된 객체입니다.
	VO 클래스는 자바빈 클래스로 생성합니다.
	
	자바빈(java bean)
	웹 프로그래밍에서 어떠한 객체를 가지고 오기 위한 기법이며, 자바 언어로 작성된 클래스를 일반적으로 부르는 호칭입니다.
...
public class BoardVO {
	...
	public String toString() {
		return "BoardVO [writer=" + writer + ", title=" + title + ", content=" + content + ", regDate=" + regDate + "]";
	}
