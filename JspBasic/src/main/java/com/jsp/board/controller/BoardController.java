...
//			/JspBasic/board/board_write.jsp // 절대
//			board/board_write.jsp // 상대
			break;
		case "regist":
		...
		case "list":
			...
			/*
				...
				여기서는 forward 방식을 사용해서 request를 원하는 jsp파일로 전달해서
				그쪽에서 응답이 나갈 수 있도록 처리해야합니다.
			 */
//			response.sendRedirect("board/board_list.jsp");
			...
			// Dispatcher에는 절대경로를 적지 말고 상대경로를 적어라
			dp.forward(request, response);
			break;
		case "content":
//			/JspBasic/content.board?bId=${status.count} 라는 요청이 들어왔을때
		...
		case "update":
//			새롭게 입력받은 수정값으로 BoardBo 객체를 생성해서 수정을 진행하세요.
//			(기존 리스트에 존재하는 객체를 새로운 객체로 교체)
//			수정이 완료되면 수정된 글의 상세보기 페이지로 응답이 가야합니다
